...
* 对于任意 ``T``：
  ...
  * 对于 ``std::string``，翻译为 JavaScript ``string``。
...
* 对于任意 ``T *`` 以及 ``std::reference_wrapper<T>``：均采用孪生方式。
...
* 该结构体使用 regType 注册，且满足 ``EJS::Translatable`` 概念
...
每当 JavaScript 代码尝试访问孪生体的属性时，ElementQuickJsBinding 会获取原对象的属性值，然后转换为 JavaScript 类型并返回给 JavaScript 代码。

每当 JavaScript 代码尝试设置孪生体的属性时（假设设置属性 prop 为 value），ElementQuickJsBinding 将首先将 value 转换为 C++ 类型，然后按照顺序查找遍历 prop 的所有 operator= 重载，直到找到一个可以将 value 转换为其参数的重载，最后调用该重载设置属性。
...
当 JavaScript 调用了函数，且函数有返回值，ElementQuickJsBinding 会如此处理返回值：

*  如果返回值为 ``T &``，则将之包装为 ``std::reference_wrapper<T>``，然后执行 ``cpp2VariantJs`` 转换。
*  如果返回值为 ``T *`` 或 ``std::reference_wrapper<T>`` 或 ``T``，则直接执行 ``cpp2VariantJs`` 转换。
...
若未满足以上条件，则该孪生体在被 JavaScript 使用时不会判断源对象是否被销毁或移动，需要用户确保内存安全。

驱动程序（Driver）
~~~~~~~~~~~~~~~~~~~~~~

ElementQuickJsBinding 提供一种机制，允许用户说明对于同一类类，均有哪些共同的函数或属性可供 JavaScript 侧调用。该功能在模板类上尤其有用。
...
.. code-block:: C++
   ...
   regClass<std::vector<A>, SeqContainerDriver>(...);
   regClass<std::vector<B>, SeqContainerDriver>(...);
