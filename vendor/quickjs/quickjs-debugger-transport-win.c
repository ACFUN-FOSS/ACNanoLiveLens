...
void js_debugger_wait_connection(JSContext *ctx, const char *address) {
    ...
    int reuseAddress = 1;
    assert(setsockopt(server, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuseAddress, sizeof(reuseAddress)) != SOCKET_ERROR);

    assert(bind(server, (struct sockaddr *)&addr, sizeof(addr)) != SOCKET_ERROR);

    assert(listen(server, 1) != SOCKET_ERROR);
